 ...
 * NULLABLE int => 是否允许使用 NULL。 columnNoNulls - 可能不允许使用NULL值， columnNullable - 明确允许使用NULL值， columnNullableUnknown - 不知道是否可使用 null<br>
 ...
 * IS_NULLABLE String => ISO 规则用于确定列是否包括 null， YES --- 如果参数可以包括 NULL， NO --- 如果参数不可以包括 NULL， 空字符串 --- 如果不知道参数是否可以包括 null<br>
 ...
 * SOURCE_DATA_TYPE short => 不同类型或用户生成 Ref 类型、来自 java.sql.Types 的 SQL 类型的源类型（如果 DATA_TYPE 不是 DISTINCT 或用户生成的 REF，则为null）<br>
 * IS_AUTOINCREMENT String => 指示此列是否自动增加，YES --- 如果该列自动增加 ， NO --- 如果该列不自动增加， 空字符串 --- 如果不能确定该列是否是自动增加参数<br>
 *
 * <pre>
 */
